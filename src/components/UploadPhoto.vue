<template>
  <div class="upload-warpper"> 
    ...
    <input id="JS_upload_input2" @click="prepare" class="file-input" type="file"  @change="fileChange" :accept="uploadType=='image'?'image/*':''" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    fileChange() {
      this.upload($("#JS_upload_input2")[0].files[0]);
    },
    upload(file) {
      let _this = this;
